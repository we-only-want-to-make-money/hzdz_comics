...
<div class="navbar flt">
    <nav class="tab-box">
        <div class="item">
            <a href="<?php echo U('Mh/book_shelf');?>">收藏</a>
        </div>
        <div class="item">
            <a href="<?php echo U('Mh/book_recent_read');?>" class="active">历史</a>
        </div>
    </nav>
    <div class="action">
    ...
    <div class="column-list" id="shelf-container">
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item">
				<a href="javascript:;" class="link">
					<div class="cover">
						<img class="img-ajax-lazy" src="<?php echo ($vo["pic"]); ?>" data-original="<?php echo ($vo["pic"]); ?>" style="background: rgb(238, 238, 238); display: block;">
					</div>
					<div class="body">
						<div class="title"><?php echo ($vo["title"]); ?></div>
						<div class="author"><?php echo ($vo["author"]); ?></div>
						<div class="text">第<?php echo ($vo["episodes"]); ?>话</div>
					</div>
				</a>
				<div class="action">
					<?php if($vo['type'] == 'mh'): ?><a href="<?php echo U('Mh/inforedit',array('mhid'=>$vo['rid'],'ji_no'=>$vo['episodes']));?>" class="btn">续看</a>
					<?php else: ?>
						<a href="<?php echo U('Book/inforedit',array('bid'=>$vo['rid'],'ji_no'=>$vo['episodes']));?>" class="btn">续看</a><?php endif; ?>
				</div>
				<div class="cp-box" style="display:none;">
					<input type="checkbox" name="edit_history_books" class="selected-switch" value="<?php echo ($vo["id"]); ?>">
...
<script type="text/javascript">
    ...
    $(".delete-selected-confirm .action .confirm").click(function(){
        ...
        var url = "<?php echo U('Mh/deleteRead');?>";
