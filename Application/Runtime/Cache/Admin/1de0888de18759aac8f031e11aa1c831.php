...
<div class="bodywrapper">
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
            <h1 class="pagetitle">漫画分类设置</h1>
            <span class="pagedesc">漫画分类设置的增加、删除、修改(若显示在主页列表，不需要设置跳转链接)</span>
        </div><!--pageheader-->
        ...
        <div id="contentwrapper" class="contentwrapper lineheight21">
            ...
            <form class="stdform stdform2" method="post">
				<div class="tableoptions">                    
                    <input value="添加一项" type="button" class="radius3" onclick="add_row()" />
                </div><!--tableoptions-->
				<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
                    <thead>
                        <tr>
							...
							<th class="head1">跳转地址</th>
							<!--th class="head1">排序权值</th-->
							<th class="head1">显示区域</th>
                    ...
                    <tbody>
                    	...
                    	<?php if(is_array($_mhcate)): $i = 0; $__LIST__ = $_mhcate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr style="background:#fff">
                            <td>
								<iframe src="<?php echo U('upload?event=setPath&cur_index='.$i.'&url='.$vo['pic']);?>" scrolling="no" style="width:100px; height:100px;"></iframe>
							...
							<td>
								<select name="show[]" default="<?php echo ($vo["show"]); ?>">
									<option value="1">分类导航</option>
									<option value="2">主页列表</option>
								</select>
							</td>
							<td>
								<select name="isshow[]" default="<?php echo ($vo["isshow"]); ?>">
				...
				function add_row(){
					...
					html = '<tr style="background:#fff">'
							+'	 <td>'
							+'		<iframe src="<?php echo U('upload?cur_index=_n_&event=setPath');?>" scrolling="no" style="width:100px; height:100px;"></iframe>'
							+'		<input type="hidden" name="pic[]" value="" />'
							+'	</td>'
							+'	<td><input type="text" name="name[]" /></td>'
							+'	<td><input type="text" name="url[]" /></td>'
							+'<td>'
							+'	<select name="show[]" default="">'
							+'		<option value="1">分类导航</option>'
							+'		<option value="2">主页列表</option>'
							+'	</select>'
							+'</td>'
							+'<td>'
							+'	<select name="isshow[]" default="">'
							+'		<option value="1">是</option>'
							+'		<option value="0">否</option>'
							+'	</select>'
							+'</td>'
					...
					size_r = size+1;
					jQuery("#table2").find("tbody").append(html.replace('_n_',size_r));
				}
