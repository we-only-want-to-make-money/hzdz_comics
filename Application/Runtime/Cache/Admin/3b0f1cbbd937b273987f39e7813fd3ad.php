...
<div class="bodywrapper">
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
            <h1 class="pagetitle">自定义菜单管理</h1>
            <span class="pagedesc">设置公众号的自定义菜单</span>
        ...
        <div id="contentwrapper" class="contentwrapper lineheight21" style="background:#fff">
			...
			<div class="tableoptions">                    
				<button class="radius3" onclick="location.href='<?php echo U('edit');?>'">添加菜单</button>
				<button class="radius3" onclick="if(confirm('自定义菜单发布成功后需要重新关注或者等一段时间才生效，确认发布？'))location.href='<?php echo U('send');?>'">发布自定义菜单</button>
			</div><!--tableoptions-->
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo (get_selfmenu_type($vo["type"])); ?></td>
						<td title="<?php echo ($vo["extra"]); ?>"><?php echo (substr($vo["extra"],0,80)); ?></td>
						<td class="center">
							<a href="<?php echo U('edit', 'id='.$vo['id']);?>">修改</a> | 
							<a href="<?php echo U('del', 'id='.$vo['id']);?>" onclick="return confirm('你确实要删除这个菜单吗？')">删除</a>
						</td>
