...
<div class="bodywrapper">
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
            <h1 class="pagetitle">漫画分集管理（<?php echo ($mhinfo["title"]); ?>）</h1>
            <span class="pagedesc">对漫画分集进行增加，修改或者删除等操作</span>
        ...
        <div id="contentwrapper" class="contentwrapper lineheight21" style="background:#fff">
			...
			<div class="tableoptions">                    
				<button class="radius3" onclick="location.href='<?php echo U('episodesedit');?>&mhid=<?php echo ($mhid); ?>'">添加漫画分集</button>
			</div><!--tableoptions-->
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
				<thead>
					<tr>
						...
						<th class="head0">编辑图片内容</th>
						<th class="head0">是否已编辑图片</th>
						<th class="head0">阅读人数</th>
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><a href="<?php echo U('episodeseditpic');?>&mhid=<?php echo ($mhid); ?>&id=<?php echo ($vo['id']); ?>">图片内容</a></td>
						<td>
							<?php if($vo['pics'] != ''): ?>是
							<?php else: ?>
								否<?php endif; ?>
						</td>
						<td><?php echo ($vo["readnums"]); ?></td>
						<td class="center">
							<a href="<?php echo U('episodesedit');?>&mhid=<?php echo ($mhid); ?>&id=<?php echo ($vo['id']); ?>">修改</a> | 
							<a href="<?php echo U('episodesdel', 'id='.$vo['id']);?>" onclick="return confirm('你确实要删除这个漫画分集吗？')">删除</a>
						</td>
