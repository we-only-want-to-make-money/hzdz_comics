...
function uploadStart(file) {
	try {
		...
		It's important to update the UI here because in Linux no uploadProgress12 events are called. The best
...
function uploadProgress12(file, bytesLoaded, bytesTotal) {
	try {
...
function uploadComplete12(file) {
	if (this.getStats().files_queued === 0) {
		document.getElementById(this.customSettings.cancelButtonId).disabled = true;
	}
}
...
function queueComplete(numFilesUploaded) {
	...
	alert(numFilesUploaded + " 个文件" + (numFilesUploaded === 1 ? "" : "") + "已上传。");
}
